...
int DigitalTwinClientHelper_SetCommandResponse(DIGITALTWIN_CLIENT_COMMAND_RESPONSE* commandResponse, const char* responseData, int status)
{
    ...
    if ((commandResponse->responseData = malloc(responseLen + 1)) == NULL)
    {
        LogError("DigitalTwinClientHelper_SetCommandResponse: Unable to allocate response data");
        commandResponse->status = 500;
        result = MU_FAILURE;
    }
    else
    {
        strcpy((char*)commandResponse->responseData, responseData);
